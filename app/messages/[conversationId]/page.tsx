  ...
  return (
    <main className="mx-auto flex h-dvh w-full max-w-4xl flex-col bg-[var(--background)] px-4 py-8 shadow-2xl shadow-[var(--line)]">
      ...
      <div className="mb-6 flex items-center gap-4 rounded-b-3xl border-b border-[var(--line)] bg-white pb-6 pt-2">
        {otherParticipant ? (
          <div className="flex h-12 w-12 items-center justify-center rounded-full bg-[#ffe4e6] text-xl font-bold text-[#be123c] shadow-sm">
            {(otherParticipant.name || "U")[0].toUpperCase()}
          </div>
        ) : (
          <div className="h-12 w-12 rounded-full bg-[#ffe4e6] animate-pulse" />
        )}
        <div>
      ...
      <div className="flex-1 overflow-y-auto px-2" ref={scrollRef}>
        ...
        {error ? (
          <div className="my-8 rounded-2xl bg-[#fff1f2] p-4 text-center text-sm font-semibold text-[#be123c]">
            {error}
          </div>
        ) : null}
              ...
              >
                <div
                  className={`relative max-w-[80%] rounded-2xl px-5 py-3 shadow-sm sm:max-w-[#70%] ${
                    isMe
                      ? "rounded-br-sm bg-[var(--brand)] text-white shadow-[#e11d48]/20"
      ...
      <form className="mt-4 flex flex-col gap-3 rounded-3xl border border-[var(--line)] bg-white p-2 shadow-lg sm:flex-row sm:p-3" onSubmit={onSubmit}>
        <input
          ...
          placeholder="Message..."
          className="flex-1 rounded-2xl bg-transparent px-4 py-3 text-[15px] text-[var(--foreground)] outline-none placeholder:text-[var(--muted)] focus:bg-[#fff7f8]"
        />
        <button
          type="submit"
